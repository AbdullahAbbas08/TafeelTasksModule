  ...
  transform(value: Date, args?: any): any {
    ...
    if (new Date(value).getTime() <= 0) { // * it will be invalid date or Hijri Date.
      ...
      let offsetTime = new Date(targetTime.getTime() + tzDifference * 60 * 1000),
        ...
        day = (offsetTime.getDate()) + (+localStorage['hijriDaysValue']);
      dateInString = `${day}/${month}/${year}`;
      return dateInString;
    };
    // console.log('customDate() @Pipe', value)
    let dateString: string,
        convDate = new Date(value),
        tzDifference = convDate.getTimezoneOffset(),
        date = new Date(convDate.getTime() + tzDifference * 60 * 1000),
    //   date = new Date(value),
