...
namespace CommiteeAndMeetings.Service.Helpers
{
    ...
    public class AuthHandler : AuthorizationHandler<HasPermissionRequirment>
    {
        ...
        protected override Task HandleRequirementAsync(AuthorizationHandlerContext context, HasPermissionRequirment requirement)
        {
            ...
            string PermissionCodeOfRequest = _unitOfWork.GetRepository<Permission>().GetAll(false)
                .Where(x => x.Url.ToUpper().Contains(RequestURL) && x.Method.ToUpper().Contains(RequestMethod)).FirstOrDefault()?.PermissionCode.ToUpper();
