...
namespace HelperServices
{
    public class SessionServices : _HelperService, ISessionServices
    {
        ...
        //For UserRole
        public int UserRoleIdOrignal
        {
            get
            {
                if (HttpContext.User == null)
                    return 0;
                var claim = HttpContext.User.FindFirst(Encrypt("UserRoleId"));
                if (claim == null)
                    return 0;
                return int.Parse(Decrypt(claim.Value));
            }
            set => HttpContext.Session.SetString("UserRoleId", value.ToString());
        }
        public int? RoleId
        {
            get
        ...
        public string Culture
        {
            ...
            set => HttpContext.Session.SetString("culture", value == null ? "ar" : value.ToString());
        }
        ...
        public void SetAuthTicket(string username, EncriptedAuthTicketDTO authTicket)
        {
            HttpContext.Session.SetString(username.ToUpper(), JsonConvert.SerializeObject(authTicket));
        }

        public EncriptedAuthTicketDTO GetAuthTicket(string username)
        {
            var Auth = HttpContext.Session.GetString(username.ToUpper());
            if (Auth != null)
                return JsonConvert.DeserializeObject<EncriptedAuthTicketDTO>(Auth);
            else
                return null;
        }

        // for return UserId and UserRoleId After Decrypt
        public  UserIdAndRoleIdAfterDecryptDTO UserIdAndRoleIdAfterDecrypt (string encryptedValue,bool flag)
        {
            int Id = 0;
            int UserRoleIdFromPara = 0;
            string IdAndUserRoleId = Encription.DecryptStringAES(encryptedValue);
            if (flag == false) //id Only
            {

                //if (!IdAndUserRoleId.Contains('_'))
                //{
                    int.TryParse(IdAndUserRoleId, out Id);

                    return new UserIdAndRoleIdAfterDecryptDTO() {Id = Id};
               // }
            }
            // send Id_RoleId 
            else 
            {
                ...
                int.TryParse(IdAndUserRoleId.Substring(indexOf + 1), out UserRoleIdFromPara);

                // transactionActionIdAfterCalling = int.Parse(transactionActionIdsplit);


                if (UserRoleIdOrignal != UserRoleIdFromPara)
                {
                    throw new UnauthorizedAccessException();
                }
                return new UserIdAndRoleIdAfterDecryptDTO() {Id =Id , UserRoleId = UserRoleIdOrignal };
            }
        }
