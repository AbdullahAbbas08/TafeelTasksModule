...
namespace GhostFactory
{
    ...
    internal class PDFConvert
    {
        ...
        private const string GS_Compression = "-sCompression={0}";
        ...
        {
            get { return _sCompression; }
            set { _sCompression = value; }
        }
        ...
        /// <returns>True if the conversion succed!</returns>
        private bool Convert(string inputFile, string outputFile, string parameters)
        {
        ...
        private bool Print(string inputFile, string printParametersFile)
        {
            #region Check Input
            //Avoid to work when the file doesn't exist
            if (string.IsNullOrEmpty(inputFile))
        ...
        private string[] GetGeneratedArgs(string inputFile, string outputFile, string[] presetParameters)
        {
            ...
            if ((presetParameters == null) || (presetParameters.Length == 0))
            {
                ...
                //
                if (!string.IsNullOrEmpty(_sCompression))
                    lstExtraArgs.Add(string.Format(GS_Compression, _sCompression));
                else
                    lstExtraArgs.Add(string.Format(GS_Compression, "lzw"));
